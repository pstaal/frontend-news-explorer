    ...
    <nav>
      <ul className={`nav ${isSavedArticlesUrl ? "nav-black" : ""}`}>
        {isLoggedIn && (
          <NavLink
            to={"/saved-news"}
            onClick={removeMobileStyling}
            className={({ isActive }) =>
              "nav__item" +
              (isSavedArticlesUrl ? " nav-black" : "") +
        ...
        {isLoggedIn ? (
          <li
            className={`nav__button ${isSavedArticlesUrl ? "nav-black" : ""}`}
            onClick={logout}
          >
            {value.name}{" "}
            <img
              ...
              src={isSavedArticlesUrl ? logoutBlackIcon : logoutWhiteIcon}
            />
          </li>
        ) : (
          <li
            className={`nav__button nav__button-loggedout ${
              isSavedArticlesUrl ? "nav-black" : ""
            }`}
            onClick={openModal}
          >
